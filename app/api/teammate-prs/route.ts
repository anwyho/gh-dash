...
export async function GET() {
  try {
    ...
    const byTeammate = await cached(`teammate-prs:${repo}`, CACHE_TTL, async () => {
      ...
      const result: Record<string, PrCardData[]> = {};
      for (const item of (data as { items: unknown[] }).items) {
        ...
        result[login].push(pr);
      }
      return result;
    });

    return NextResponse.json({ byTeammate, lastFetchedAt: new Date().toISOString() });
  } catch (err) {
